...
总体上，binder 生成遵循“\emph{接触识别-形态分类-局部几何构造-体积分数校准}”的流程：首先在纤维几何集合中识别近接触纤维对并确定最近点对作为锚点；随后根据两纤维相对取向（共面胶桥/异面胶膜的区别）构造不同形态的三维局部 binder 体积；接着将其体素化写入 $B(\textbf x)$ 并保持与纤维互斥（$B(\textbf x) \cdot S(\textbf x) = 0$）；最后通过调整局部几何或迭代生成，使 $\phi_{b}$ 逼近目标 $\phi_{b,\textbf{target}}$，并满足整体孔隙率约束 $|\varepsilon - \varepsilon_{\textbf{target}}| \le \delta_{\varepsilon}$。
...
与粘连剂 binder 生成算法相关的关键参数详见 Settings 中表 3。
...
与此同时，为了降低体素边界带来的锯齿感并获得更平滑的等值面显示，本文额外构造用于可视化的连续场 $\tilde{S}(\textbf{x})$ 和 $\tilde{B}(\textbf{x})$，取值范围为 $[0,1]$，其可由二值相场经局部平滑/软边界映射得到（具体详见 2.4 节）。并定义合成显示场
\begin{equation}
